...
$("#playlist #overlays.ytd-thumbnail #progress").parents("ytd-rich-item-renderer").show();
...
/** getting logged youtube ID **/
function gotyoutubeid(item) {window.loggedprofile = `${item.youtubeid.name}`;}
browser.storage.local.get("youtubeid")
.then(gotyoutubeid, onError);

/** getting PLAYLISTS state **/
function TOBEHIDDENxPLAYLISTSxFUN(item) {window.PLAYLISTSstate = `${item.TOBEHIDDENxPLAYLISTS.value}`}
browser.storage.local.get("TOBEHIDDENxPLAYLISTS")
  .then(TOBEHIDDENxPLAYLISTSxFUN, onError);

/** getting CHANNELS state **/
function TOBEHIDDENxCHANNELSxFUN(item) {window.CHANNELSstate = `${item.TOBEHIDDENxCHANNELS.value}`}
browser.storage.local.get("TOBEHIDDENxCHANNELS")
  .then(TOBEHIDDENxCHANNELSxFUN, onError);

/** getting others state **/
function TOBEHIDDENxOTHERSxFUN(item) {window.OTHERSstate = `${item.TOBEHIDDENxOTHERS.value}`}
...
if (window.location.href.indexOf('www.youtube.com/playlist?list=') != -1 && window.PLAYLISTSstate ='on' /** do just if playlists hidding is on**/) {
...
if (parseInt(progressfullvl) >= `${item.confperc.value}`) {$(this).parents("ytd-playlist-video-renderer").hide();}
})
...
function channel(){
if (window.location.href.indexOf('channel/') != -1 && window.CHANNELSstate ='on' /** do just if channel hidding is on**/) {
...
function watch(){
if (window.location.href.indexOf('www.youtube.com/watch?v=') != -1 && window.PLAYLISTSstate ='on' /** do just if playlists hidding is on**/) {
